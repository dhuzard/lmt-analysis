...
def click_me():  # Event handler = Start when the "Validate choice" button is clicked
    ...
    # TODO: Try to automatize the analysis depending on the nbAnimals in the file analyzed !!!
    # Dax: avoid repeating the "if nbAnimals" several times...
...
if __name__ == '__main__':
    ...
    animalList = [0]
    animalIdList = ["No animal"]
    print(animalPool.getAnimalsDictionnary())
    nbAnimals = 0
    ...
    print("animal baseID :", animalIdList)
    print("Number of animals :", nbAnimals)

    # *********** WIDGET ************
    widget = tk.Tk()  # Create the widget
    ...
    # Set the Widget depending on the number of animals in the recording
    if nbAnimals == 1:  # 1 animal in LMT
    ...
    elif nbAnimals == 2:  # 2 animals in LMT
        ttk.Label(widget, text="Mouse A:").grid(row=1, column=0)
        mouseAChosen = ttk.Combobox(widget, width=60, state='readonly', values=animalList)
        mouseAChosen.grid(row=1, column=1)
        mouseAChosen.current(0)
    ...
    elif nbAnimals == 3:  # 3 animals in LMT
        ttk.Label(widget, text="Mouse A:").grid(row=1, column=0)
        mouseAChosen = ttk.Combobox(widget, width=60, state='readonly', values=animalList)
        mouseAChosen.grid(row=1, column=1)
        mouseAChosen.current(0)
    ...
    elif nbAnimals == 4:  # 4 animals in LMT
        ttk.Label(widget, text="Mouse A:").grid(row=1, column=0)
        mouseAChosen = ttk.Combobox(widget, width=60, state='readonly', values=animalList)
        mouseAChosen.grid(row=1, column=1)
        mouseAChosen.current(0)
        ...
        print("Error ?!? More than 4 animals in LMT ??")
