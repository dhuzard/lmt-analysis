...
def getNumberOfEventWithList(connection, file, eventName, idAnimalA, animalList, minFrame=None, maxFrame=None):
    ...
    for animalCandidate in animalList:
        timeLine = EventTimeLineCached(connection, file, eventName, idAnimalA, animalCandidate.baseId,
...
if __name__ == '__main__':
    ...
    behaviouralEventTwoMice = ["Approach contact", "Approach rear", "Break contact", "Contact", "FollowZone Isolated",
                               "Group2", "Oral-oral Contact", "Oral-genital Contact", "Side by side Contact",
                               "Side by side Contact, opposite way", "Social approach", "Get away", "Train2"]
        ...
        for eventNight in nightEventTimeLine.getEventList():
            ...
            for behavEvent in behaviouralEventTwoMice:
                ...
                for idAnimalA in pool.animalDictionnary:
                    ...
                    nbEventsSameGeno = getNumberOfEventWithList(connection, file, behavEvent, idAnimalA, animalSameGeno,
                                                                minFrame=minT, maxFrame=maxT)
                    durEventsSameGeno = getDurationOfEventWithList(connection, file, behavEvent, idAnimalA,
                                                                   animalSameGeno, minFrame=minT, maxFrame=maxT)

                    nbEventsDiffGeno = 0
                    durEventsDiffGeno = 0

                    if MODE_MULTI_GENOTYPE == True:
                        nbEventsDiffGeno = getNumberOfEventWithList(connection, file, behavEvent, idAnimalA,
                                                                    animalDiffGeno, minFrame=minT, maxFrame=maxT)
                        durEventsDiffGeno = getDurationOfEventWithList(connection, file, behavEvent, idAnimalA,
                                                                       animalDiffGeno, minFrame=minT, maxFrame=maxT)

                    print(behavEvent, pool.animalDictionnary[idAnimalA].RFID)

                    resSame = [file, behavEvent, pool.animalDictionnary[idAnimalA].RFID,
                               pool.animalDictionnary[idAnimalA].genotype, "sameGeno", durEventsSameGeno,
                               nbEventsSameGeno]
                    # text_file.write( "{}\n".format( resSame ) )
                    if MODE_MULTI_GENOTYPE == True:
                        resDiff = [file, behavEvent, pool.animalDictionnary[idAnimalA].RFID,
                                   pool.animalDictionnary[idAnimalA].genotype, "diffGeno", durEventsDiffGeno,
                                   nbEventsDiffGeno]
                        # text_file.write( "{}\n".format( resDiff ) )
            ...
            for behavEvent in behaviouralEventTwoMice:
                ...
                for idAnimalA in pool.animalDictionnary.keys():
                    for idAnimalB in pool.animalDictionnary.keys():
                        ...
                        event = EventTimeLineCached(connection, file, behavEvent, idAnimalA, idAnimalB, minFrame=minT,
                                                    maxFrame=maxT)
                        ...
                        resSame = [file, behavEvent, pool.animalDictionnary[idAnimalA].RFID,
                                   pool.animalDictionnary[idAnimalB].RFID, minT, maxT, totalEventDuration, nbEvent]

                        text_file.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(file, behavEvent,
                                                                                          pool.animalDictionnary[
                                                                                              idAnimalA].RFID,
                                                                                          pool.animalDictionnary[
                                                                                              idAnimalA].genotype,
                                                                                          pool.animalDictionnary[
                                                                                              idAnimalB].RFID,
                                                                                          pool.animalDictionnary[
                                                                                              idAnimalB].genotype, minT,
                                                                                          maxT, totalEventDuration,
                                                                                          nbEvent))
