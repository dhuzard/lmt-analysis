...
from lmtanalysis import BuildEventTrain3, BuildEventTrain4, BuildEventFollowZone, BuildEventRear5, \
...
if __name__ == '__main__':
    ...
    behaviouralEventOneMouse = ["Group3", "Group 3 break", "Group 3 make", "Move isolated", "Move in contact",
                                "Rearing", "Rear isolated", "Rear in contact", "Stop isolated", "WallJump"]
    behaviouralEventTwoMice = ["Approach contact", "Approach rear", "Break contact", "Contact", "Group2",
                               "Oral-oral Contact", "Oral-genital Contact", "Side by side Contact",
                               "Side by side Contact, opposite way", "Social approach", "Social escape", "Train2"]
    ...
    for file in files:
        ...
        for behavEvent in behaviouralEventOneMouse:
            ...
            for idAnimalA in pool.animalDictionnary.keys():
                behavEventTimeLine[idAnimalA] = EventTimeLine(connection, behavEvent, idAnimalA, minFrame=tmin,
                                                              maxFrame=tmax)
                ...
                resOneMouse = [file, event.eventName, pool.animalDictionnary[idAnimalA].RFID, genoA, totalEventDuration,
                               nbEvent]
                text_file.write(
                    "{}\t{}\t{}\t{}\t{}\t{}\n".format(file, event.eventName, pool.animalDictionnary[idAnimalA].RFID,
                                                      genoA, totalEventDuration, nbEvent))
        ...
        for behavEvent in behaviouralEventTwoMice:
            ...
            for idAnimalA in pool.animalDictionnary:
                ...
                nbEventsB6Geno = getNumberOfEventWithList(connection, behavEvent, idAnimalA, animalB6Geno,
                                                          minFrame=tmin, maxFrame=tmax)
                durEventsB6Geno = getDurationOfEventWithList(connection, behavEvent, idAnimalA, animalB6Geno,
                                                             minFrame=tmin, maxFrame=tmax)
                nbEventsDiffGeno = getNumberOfEventWithList(connection, behavEvent, idAnimalA, animalDiffGeno,
                                                            minFrame=tmin, maxFrame=tmax)
                durEventsDiffGeno = getDurationOfEventWithList(connection, behavEvent, idAnimalA, animalDiffGeno,
                                                               minFrame=tmin, maxFrame=tmax)

                print(behavEvent, pool.animalDictionnary[idAnimalA].RFID)

                resSame = [file, behavEvent, pool.animalDictionnary[idAnimalA].RFID,
                           pool.animalDictionnary[idAnimalA].genotype, "B6", durEventsB6Geno, nbEventsB6Geno]
                resDiff = [file, behavEvent, pool.animalDictionnary[idAnimalA].RFID,
                           pool.animalDictionnary[idAnimalA].genotype, "diffGeno", durEventsDiffGeno, nbEventsDiffGeno]
                text_file.write(
                    "{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(file, behavEvent, pool.animalDictionnary[idAnimalA].RFID,
                                                          pool.animalDictionnary[idAnimalA].genotype, "B6",
                                                          durEventsB6Geno, nbEventsB6Geno))
