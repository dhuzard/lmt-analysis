...
class Night(object):
    ...
    def nextDay(self):
        ...
        currentStartDay = datetime.datetime(int(currentStartDay.split("-")[0]), int(currentStartDay.split("-")[1]),
                                            int(currentStartDay.split("-")[2]))
        currentEndDay = datetime.datetime.strftime(self.endDate, "%Y-%m-%d")
        currentEndDay = datetime.datetime(int(currentEndDay.split("-")[0]), int(currentEndDay.split("-")[1]),
                                          int(currentEndDay.split("-")[2]))
...
def insertNightEvent(file):
    """
    This function creates the 'Night' events
    """
    ...
    print("Flushing the event from the database")
    flush(connection)
    print("------ Flush Done ------")
    ...
    """
    Two cases: 
    - end night hour < start night hour: it means that the night ends on the day after.
    - end night hour > start night hour: it means that the night is during daytime => reversed cycle.
    """
