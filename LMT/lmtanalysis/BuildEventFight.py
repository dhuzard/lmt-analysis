...
def getMaxDistanceTo(animal, target, startFrame, endFrame):
        '''
        returns the maximum distance from this detection to detectionB,
        considering time window
        '''
...
def reBuildEvent(connection, file, tmin=None, tmax=None, pool=None):
    ...
    Based on the number of particles (Dax: bedding projections due to fighting?)
    ...
    # work only with 2 animals at the moment. Should test with 3 or 4 later on.
    ...
    contactDictionnary = EventTimeLineCached(connection, file, "Contact",
                                             minFrame=tmin, maxFrame=tmax).getDictionnary(tmin, tmax)

    followZoneTimeLine = {}
    followZoneTimeLine[1, 2] = EventTimeLineCached(connection, file, "FollowZone Isolated",
                                                   idA=1, idB=2, minFrame=tmin, maxFrame=tmax)
    followZoneTimeLine[2, 1] = EventTimeLineCached(connection, file, "FollowZone Isolated",
                                                   idA=2, idB=1, minFrame=tmin, maxFrame=tmax)

    moveEventTimeLine = {}
    moveEventTimeLine[1] = EventTimeLineCached(connection, file, "Move",
                                               idA=1, minFrame=tmin, maxFrame=tmax)
    moveEventTimeLine[2] = EventTimeLineCached(connection, file, "Move",
                                               idA=2, minFrame=tmin, maxFrame=tmax)
    ...
    for event in fightTimeLine.getEventList():
        ...
        if closestEventTDistance < delayAfterFight:
            # means another fight is occurring. This one is not finished yet.
            continue
    ...
    print("Rebuild Fight event finished.")
