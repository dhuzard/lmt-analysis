...
def checkZone(massA_x, massA_y, angleA, meanSizeB, massB_x, massB_y, angleB):
    ...
    if dif < math.pi / 4:
        ...
        if -0.5 * meanSizeB < x < 0.5 * meanSizeB and -meanSizeB < y < 0:
...
def reBuildEvent(connection, file, tmin=None, tmax=None, pool=None):
    ...
    - the mass center of the follower is within a follow zone of one mean body length of width and 
      two mean body lengths of length.
    - either the animal B is in contact with another one (FollowZone Social) or 
      the animal B is not in contact with any other animal (except A at the end of the follow event; FollowZone Isolated)
    
    - updated. 17 may 2018: now works with n animals
    """
    ...
    for idAnimalB in pool.animalDictionnary.keys():
        ...
        for idAnimalA in pool.animalDictionnary.keys():
            if idAnimalB is idAnimalA:
                continue

            contact[idAnimalA, idAnimalB] = EventTimeLineCached(connection, file, "Contact",
                                                                idAnimalA, idAnimalB,
    ...
    for idAnimalB in pool.animalDictionnary.keys():
        ...
        for idAnimalA in pool.animalDictionnary.keys():
            ...
            for t in dicB.keys():
                ...
                if testContactWithOtherAnimal is True:
                    continue
                ...
                if t in dicA.keys():
                    ...
                    if speedA is None or speedB is None:
                        continue
                    ...
                    if speedA > SPEED_THRESHOLD_LOW and speedB > SPEED_THRESHOLD_LOW:
                        ...
                        for time in range(t - 15, t + 1, 2):
                            ...
                            try:
                                angleB = pool.animalDictionnary[idAnimalB].getDirection(time)
                                if checkZone(dicA[t].massX, dicA[t].massY, angleA, meanSizeB, dicB[time].massX,
                                             dicB[time].massY, angleB) is True:
                                    resultIso[t] = True
                                    break
                            except:
                                pass
    ...
    print("Rebuild event Follow (in zone) finished.")
