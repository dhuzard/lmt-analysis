...
class TaskLogger:
    ...
    def addLog(self, process, version='0', date=None, tmin=0, tmax=-1):
        ...
        cursor = self.conn.cursor()
        query = "INSERT INTO LOG(process, version, date, tmin, tmax) VALUES ('{process}', '{version}', '{date}', " \
                "'{tmin}', '{tmax}')".format(process=process, version=version, date=date, tmin=tmin, tmax=tmax)
