...
def reBuildEvent(connection):
    """
    Event SideWalk: Walk on the side
    ...
    for idAnimalA in pool.animalDictionnary.keys():
        ...
        for t in dicA.keys():
            ...
            if speedX is None or speedY is None:
                continue
        ...
        if len(sideWalkTimeLine.eventList) == 0:
            ...
            print("Number of events: ", len(sideWalkTimeLine.eventList))
            print("Mean length of events: ", sideWalkTimeLine.getMeanEventLength())
            print("first event frame: ", sideWalkTimeLine.timeLineList[0].startFrame)

        print("Delete old entry in base: " + eventName)
        deleteEventTimeLineInBase(connection, eventName, idAnimalA, None, None,
                                  None)  # None is assumed as the name of the event contains the id description
    ...
    print("Rebuild SideWalk event finished.")
